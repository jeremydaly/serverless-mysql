...
This module works with all MySQL versions, including MySQL 5.x and MySQL 8.x. When using MySQL 8.4+ (LTS), you may encounter different error messages when authentication fails (e.g., when using `changeUser` with invalid credentials). This is because newer MySQL versions have deprecated the `mysql_native_password` authentication plugin in favor of more secure methods. The module handles these differences automatically, but you may see different error codes in your logs.
